...
export function FeeDetail({
  ...
  const contentTypeSize = fileList?.reduce(
    (pre, item) =>
      pre + item?.dataURL?.split('data:')[1]?.split('base64')[0]?.length,
    0,
  ); // the size of contentType
  const feeFileSize = fileList
    ?.slice(0, 25)
    ?.reduce((pre, item) => pre + item?.size, 0); // the total size of first 25 files
  ...
  return (
    <Box borderTop="1px" borderBottom="1px" borderColor="black" py={ 2 } mt={ 4 }>
      <Box
        display="flex"
      ...
      >
        ...
        >
          <del>{ total }</del>
          <Box color="#E75F00" fontWeight="bold">
            { amount }
          </Box>
        </Box>
      </Box>
